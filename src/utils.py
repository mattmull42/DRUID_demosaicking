...
def format_output(
    output: list[list[torch.Tensor]], crop: bool = True
...
def plot_psnr_stages(
    gt_list: torch.Tensor, x_hat_list: torch.Tensor, cfa: list[str], cfa_idx: list[int]
) -> None:
    if len(cfa) != len(cfa_idx):
        for i in range(len(gt_list)):
            plt.plot(
                [psnr(gt_list[i], x_hat) for x_hat in x_hat_list[:, i]],
...
def plot_results(
    gt_list: torch.Tensor, x_hat_list: torch.Tensor, cfa: list[str], cfa_idx: list[int], stage: int
) -> None:
    ...
    for i in range(nb_images):
        ...
        if len(cfa) != len(cfa_idx):
            ax.set_title(
                f"CFA: {cfa[cfa_idx[i]]} v{i - cfa_idx.index(cfa_idx[i])}, "
...
def translation(pattern: torch.Tensor, bottom: int, right: int) -> torch.Tensor:
    trans = get_mask(
        pattern, (pattern.shape[0], pattern.shape[1] + bottom, pattern.shape[2] + right)
    )
...
def get_variants(
    pattern: torch.Tensor, out_shape: tuple[int, int, int], depth: int
) -> list[torch.Tensor]:
    res = [pattern]
    ...
    for _ in range(depth):
        for pattern_idx in range(len(res)):
            res += [
                translation(res[pattern_idx], i, j)
